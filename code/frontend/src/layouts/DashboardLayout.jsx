  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row font-sans">
            ...
            if (item.comingSoon) {
              return (
                <div
                  ...
                  className="flex items-center justify-between px-4 py-3 rounded-xl text-sm font-semibold opacity-40 cursor-not-allowed select-none"
                >
                  <div className="flex items-center space-x-3">
                    <span className="text-slate-500">{item.icon}</span>
                    <span className="text-slate-400">{item.label}</span>
                  </div>
                  <span className="text-[9px] font-bold text-slate-500 bg-slate-800 px-1.5 py-0.5 rounded-md tracking-wider uppercase">Bientôt</span>
                </div>
              );
            }
            return (
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-850 z-30 flex items-center justify-around px-4 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
        {/* Maximum 4 liens actifs pour la navigation mobile */}
        {filteredNavItems.filter(i => !i.comingSoon).slice(0, 4).map((item, index) => {
          const isActive = location.pathname === item.path;
          return (
            <Link
              key={index}
