...
const OPP_STATUSES = ['Prospect', 'Qualification', 'Proposition', 'NÃ©gociation', 'GagnÃ©e', 'Perdue'];
...
const OpportunityForm = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      showToast(isEdit ? 'OpportunitÃ© mise Ã  jour.' : 'OpportunitÃ© crÃ©Ã©e avec succÃ¨s.');
  ...
  if (isEdit && isFetching) return <LoadingSpinner fullPage message="Chargement de l'opportunitÃ©..." />;

  return (
    <div className="space-y-6 font-sans">
      <PageHeader
        title={isEdit ? "Modifier l'OpportunitÃ©" : 'Nouvelle OpportunitÃ©'}
        breadcrumb={[{ label: 'CRM' }, { label: 'OpportunitÃ©s', href: '/dashboard/opportunities' }, { label: isEdit ? 'Modifier' : 'Nouveau' }]}
      />

      <form onSubmit={handleSubmit} className="max-w-2xl space-y-5">
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
          <h3 className="text-sm font-extrabold text-slate-800 border-b border-slate-100 pb-3">DÃ©tails de l'OpportunitÃ©</h3>
          ...
          <div>
            ...
            <input type="text" name="title" value={form.title} onChange={handleChange} required placeholder="Travaux de peinture extÃ©rieure..." className="block w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-[#C85A2A] transition-all" />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-1.5">Budget estimÃ© (DH HT) <span className="text-red-500">*</span></label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-1.5">ProbabilitÃ© de succÃ¨s : {form.probability}%</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-1.5">Date de clÃ´ture prÃ©vue <span className="text-red-500">*</span></label>
        ...
        <div className="flex items-center space-x-3">
          <button type="submit" disabled={mutation.isLoading} className="py-2.5 px-6 rounded-xl bg-[#C85A2A] hover:bg-blue-500 text-white font-bold text-xs tracking-wide shadow-md shadow-blue-900/10 transition-all disabled:opacity-50 cursor-pointer">
            {mutation.isLoading ? 'Enregistrement...' : isEdit ? "Mettre Ã  jour" : "CrÃ©er l'OpportunitÃ©"}
          </button>
